...
void sphereSystem::setup(){
	...
	// timeline
	timeline.setup();
	timeline.setDurationInFrames(60*60*3);
...
void sphereSystem::manualDraw(ofEventArgs &args){
	...
	if(!bGlobalMute){
		...
		for(int i=0; i<row; i++){
			for(int j=0; j<col; j++){
				epi[i*col+j].draw(j*dw, i*dh, 0, 160);
			}
		}
        glPopMatrix();
...
void sphereSystem::receiveTrigger(ofxTLTriggerEventArgs& t){
	...
	// search spiroData directory and load param
	ofDirectory dir;
	...
	for(int i=0; i<n; i++){
		...
		if(spiro+".xml" == file){
			epi[slotNum].loadFromXml(spiro+".xml");
			return;
		}
	}
	cout << "cant find any task for slot:" + slot + ", triggerName: " + spiro << endl;
}
